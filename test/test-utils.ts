...
import { type IAstMapper, astMapper } from '../src/ast-mapper';
import { tracking } from '../src/lexer';
import { normalizeInterval } from '../src/literal-syntaxes/interval-builder';
import { parseIntervalLiteral } from '../src/parser';
import type {
    Expr,
    SelectStatement,
...
import grammar from '../src/syntax/main.ne';
import { toSql, type IAstToSql } from '../src/to-sql';
import { isFiniteNumber, trimNullish } from '../src/utils';
...
function hideLocs(val: unknown): unknown {
    if (!val) {
        return val;
    }
...
function deepEqual<T>(a: T, b: T, strict?: boolean, depth = 10, numberDelta = 0.0001) {
    ...
    const fa = isFiniteNumber(a);
    const fb = isFiniteNumber(b);
    if (fa || fb) {
...
function checkTree<T>(
    value: string | string[],
    expected: T,
    mapper: (parsed: T, m: IAstMapper | IAstToSql) => unknown,
    start?: string,
    ...
    for (const sql of value) {
        it('parses ' + sql, () => {
            ...
            function doParse(psql: string) {
                ...
                parser.feed(psql);
                const ret = <unknown[]>parser.finish();
                if (!ret.length) {
                ...
                if (ret.length !== 1) {
                    ...
                    if (noLocs.slice(1).every((p) => deepEqual(p, noLocs[0]))) {
                        assert.fail(`${noLocs.length} ambiguous syntaxes, but they yielded the same ASTs : ` +
                            inspect(noLocs[0]));
                    } else {
            ...
            expect(parsed).to.deep.equal(expected, 'Parser has not returned the expected AST');

            // check that top-level statements always have at least some kind of basic position
            expect(parsedWithLocations).to.have.property('_location', 'Top level statements must have a location');

            // check that it generates the same with/without location tracking
            expect(hideLocs(parsedWithLocations)).to.deep.equal(
                hideLocs(parsedWithoutTracking),
                'Parser did not return the same thing with and without location tracking enabled',
            );
            ...
            expect(modified).to.equal(
                parsed,
                'It is not stable when passing through a neutral AST mapper -> ' +
                'Should return THE SAME REFERENCE to avoid copying stuff when nothing changed.',
            );
