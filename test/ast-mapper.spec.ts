...
import { astMapper, assignChanged, type MapperBuilder } from '../src/ast-mapper';
import { parse as _parse, parseFirst } from '../src/parser';
import type { Expr, Statement } from '../src/syntax/ast';
import { toSql } from '../src/to-sql';

describe('Ast mapper', () => {
    function parse(str: string): Statement {
        const [first, ...rest] = _parse(str);
    ...
    it('maps ternaries', () => {
        testExpr('select a between b and 42', (_b) => ({
            ternary: (c) => c.hi,
        }), {
            type: 'integer',
            value: 42,
    ...
    it('maps case', () => {
        testExpr('select case a when b then 1 end', (_b) => ({
