import { type Token, compile, keywords } from 'moo';

import { sqlKeywords } from './keywords';
import type { NodeLocation, PGComment } from './syntax/ast';
    ...
    eString: {
        ...
        value: (x) => {
            return x.substring(2, x.length - 1).
                replace(/''/g, '\'').
                replace(/\\([\s\n])/g, (_, y: string) => y).
                replace(/\\./g, (m) => <string>JSON.parse('"' + m + '"'));
        },
    },
    qparam: {
        match: /\$\d+/,
...
lexer.next = ((next) => () => {
    let tok: (Token & { _location?: NodeLocation }) | undefined;
    let commentFull: {
        nested: number;
        offset: number;
        text: string;
...
export function box(xs: any, value: any, isDoubleQuoted?: boolean) {
    if (!trackingLoc && !isDoubleQuoted) {
        return value;
    }
    return track(xs, { [literal]: value, [doubleQuotedSym]: isDoubleQuoted });
}
